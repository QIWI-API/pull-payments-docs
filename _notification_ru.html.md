...
result_code|Код результата обработки уведомления (целое положительное число).
Рекомендуется возвращать коды результата в соответствии с [таблицей кодов завершения](#notify_codes).
...
Логин равен [ID магазина](#auth_param). Для получения пароля нажмите кнопку "Сменить пароль оповещений" в [Личном кабинете мерчанта](http://ishop.qiwi.com)в разделе "Настройки"->"REST-Протокол".
