...
Для просмотра примера PHP-скрипта выставления счета и перенаправления на Платежную форму QIWI откройте вкладку _PHP_ справа. Данный пример иллюстрирует, когда и где в запросах к Visa QIWI Wallet используются [авторизационные данные](#auth_param) провайдера, а именно, идентификатор магазина, API ID и пароль к API.
